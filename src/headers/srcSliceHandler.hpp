    ...
    srcSliceHandler(SliceDictionary *dict)
    {
        ...
        process_map = {
            ...
            {"parameter_list", [this]() {
                 ...
                 if (triggerField[constructordecl])
                 { //For the case where we need to get a constructor decl
        ...
        process_map3 = {
            ...
            {"modifier", [this]() {
                 ...
                 { //only care about modifiers in decls
                     potentialAlias = true;
                 }
...
#pragma GCC diagnostic ignored "-Wunused-parameter"
    ...
    */
    ...
    virtual void charactersUnit(const char *ch, int len)
    {
        ...
        if (((triggerField[function] || triggerField[constructor] || triggerField[destructor]) && triggerField[name]) && !(triggerField[functionblock] || triggerField[parameter_list] || triggerField[argument_list] || triggerField[argument_list_template] || triggerField[type] || triggerField[index] || triggerField[preproc] || triggerField[op] || triggerField[macro]))
        {
            currentFunctionBody.first.append(ch, len);
            //std::cerr<<currentFunctionBody.first<<std::endl;
        }
        if (triggerField[type] && triggerField[function] && !(triggerField[functionblock] || triggerField[op] || triggerField[argument_list] || triggerField[argument_list_template] || triggerField[templates] || triggerField[parameter_list] || triggerField[macro] || triggerField[preproc]))
        {
        ...
        if ((triggerField[functiondecl] || triggerField[constructordecl]) && triggerField[name] && !((triggerField[functionblock] || triggerField[parameter_list] || triggerField[argument_list] || triggerField[argument_list_template] || triggerField[type] || triggerField[index] || triggerField[preproc] || triggerField[op] || triggerField[macro])))
        {
            currentFunctionDecl.first.append(ch, len);
        }
        if ((triggerField[param] && (triggerField[function] || triggerField[functiondecl] || triggerField[constructor]) && triggerField[name]) && !(triggerField[type] || triggerField[argument_list] || triggerField[templates] || triggerField[macro] || triggerField[preproc]))
        {
            currentParam.first.append(ch, len);
        }
        if ((triggerField[param] && (triggerField[function] || triggerField[functiondecl] || triggerField[constructor])) && triggerField[name] && triggerField[type] && !(triggerField[argument_list] || triggerField[templates] || triggerField[op] || triggerField[macro] || triggerField[preproc]))
        {
        ...
        if ((triggerField[name] || triggerField[op]) && (triggerField[decl_stmt] || triggerField[control]) && triggerField[decl] && !(triggerField[argument_list] || triggerField[index] || triggerField[preproc] || triggerField[type] || triggerField[macro]))
        {
            if (triggerField[init])
            {
                ...
                else
                {
                    ...
                    if (triggerField[argument])
                    { //if it's in a call, ignore until we hit an argument
                        currentDeclInit.first.append(ch, len);
                    }
                }
            }
